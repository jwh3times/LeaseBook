...
- **`AppFolioProfiles.For(EntityKind)`** — the `appfolio-default` profiles (one per entity kind).
  Header candidates are best-guess strings (see §4 below). Plugging in the beta customer's real
...
The concrete AppFolio column header strings are **not validated**. The beta customer relationship
exists; real export files are not yet in hand. M7 ships:
...
with known-good figures) rather than the beta customer's real exports. The real cutover run on a
...
string. At the target scale (≤ 300 units, ~150-unit beta customer) this is not a constraint. If a
