...
The a11y spec (`a11y.spec.ts`) asserts zero WCAG 2 AA violations on every routed page using
`@axe-core/playwright`. It runs before `m7-onboarding` in file-discovery order, so the cutover
org's `/onboarding` route is still empty when the a11y scan hits it — this is intentional and
requires no special sequencing on your part.
