...
| Organization isolation | Single DB, `org_id` (the PM company) on every organization-scoped row; EF query filters + Postgres Row-Level Security as two independent enforcement layers — full design in "Organization isolation & RLS" below                                                                                                                                                                                                                                                                                   | Default                     |
...
## Organization isolation & row-level security (designed before M0 code)

The unit of isolation is the **organization** (one PM company). Trust data is exactly the kind of data
where a cross-organization leak is an existential bug, not a cosmetic one — so isolation is enforced
...
- **Session context mechanism:** policies read `current_setting('app.org_id', true)::uuid`.
  ...
  Npgsql reuses physical connections, and a session-level `SET` would leak organization context to the
