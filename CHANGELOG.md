...
- **Choosing a bank account in the ⌘K palette now opens that account's register.** It previously
  ...
  behind rather than carrying it to the other account.
...
- **Every screen that fails to load now says why, shows a support reference, and offers a Retry.**
  Nineteen places across the dashboard, banking, the tenant ledger, reports, settings, migration
  ...
  place instead of requiring a page reload. Two related problems are fixed with them: tenant, owner
  and property screens were substituting an invented message for whatever the server actually said,
  ...
  guessing "you may need admin rights" when the server has already said what went wrong, and the
  New bank account form and the lease late-fee overrides do the same. A page
...
- **Architecture guards now inspect compiled behavior where possible** — the error-contract and
  ...
  longer break enforcement. The remaining cross-language and source-backed guards share one
  repository reader with explicit vacuity controls, keeping their file scope and diagnostics
  consistent.

- **TypeScript 7 and generated Hey API SDK** — the SPA now compiles with native TypeScript 7 and
  consumes named request functions and models generated by Hey API instead of
  `openapi-typescript`/`openapi-fetch`. The existing build-time contract drift gate remains enforced;
  the generator is isolated on TypeScript 5 until its CLI no longer depends on compiler APIs removed
  in TypeScript 7. See [ADR-030](docs/adr/ADR-030-hey-api-and-typescript-7.md).
