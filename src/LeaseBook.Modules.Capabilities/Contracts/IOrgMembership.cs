...
/// — login happens before org context exists — so nothing in the database stops a cohort rule naming
/// a user from another organization. Both the user id and the org id are load-bearing: an id-only
/// lookup would admit a foreign tenant's user into this org's cohort. Stating it as a port keeps the
