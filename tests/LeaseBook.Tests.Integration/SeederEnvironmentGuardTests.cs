...
/// <summary>
/// Security guard: the demo, cutover and load seeders each provision a well-known,
/// source-committed admin password, so they must refuse to run in Production — provisioning any of them
/// in a reachable environment would be an account-takeover vector. Real orgs come from the M7 onboarding
...
public sealed class SeederEnvironmentGuardTests
{
    ...
    [Fact]
    public async Task LoadSeeder_refuses_to_run_in_production()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var services = BuildServices("Production");

        var ex = await Should.ThrowAsync<InvalidOperationException>(
            () => LoadSeeder.SeedAsync(services, ct));
        ex.Message.ShouldContain("Production");
    }
