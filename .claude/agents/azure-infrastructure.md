...
| `vault`      | `modules/vault.bicep`        | Key Vault (`standard`/`A`; RBAC auth; soft delete, 90-day retention; `enablePurgeProtection` prod only — one-way) |
...
- YAML keys are matched **case-sensitively** and anything unrecognised is discarded with **no error**. `secretRef` binds; `secretref`, `secret_ref` and `SecretRef` do not, and nor do `Name` or `Value`. The result is an env var with no value, whose symptom is identical to an unwired Key Vault secret — so copy `infra/jobs/capabilities-exec.yaml`, never retype it. `CapabilitiesJobTemplateTests` pins that file, and `deploy-prod.yml`'s migrator flags, against `containerapp.bicep`.
