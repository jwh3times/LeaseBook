...
`database.bicep` sets the Flexible Server `azure.extensions` allowlist to `btree_gist,pg_trgm` before
...
| **The `azure.extensions` configuration accepts `btree_gist,pg_trgm` before migrations run**                                                                                                  | Bicep validates the resource shape but only the PostgreSQL resource provider confirms the server parameter and extension availability.                                                                                                                                                             |
